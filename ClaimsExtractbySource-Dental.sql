...
select
	...
	hd.DATE as [Service Date],
	'Dental' as [Service Description],
	hd.DEDUCTIBLE as [Deductible Amount],
	hd.ELIGIBLE as [Eligible Amount],
	hd.BENEFIT as [Paid Amount],
	hd.CODE as [Procedure Code],
	hd.SURFACES as [Tooth Surface],
	hd.TOOTH as [Tooth Code]

	
into #TempClaimsExtract
from claim cl
	inner join claimdd hd on
...
where
	cl.CLAIM_TYPE = 'D'
	and len(hd.DATE) > 5
	and convert(datetime,hd.DATE,101) between @FromDate and @ToDate
